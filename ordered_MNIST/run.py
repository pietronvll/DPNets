...
import numpy as np
import torch
from data_pipeline import ClassifierFeatureMap, ClassifierModule, CNNEncoder
...
def evaluate_model(model: BaseModel, oracle: TrainableFeatureMap, test_data):
    ...
    report = {"accuracy": [], "label": [], "image": [], "times": []}
...
def run_VAMPNets():
    ...
    for rng_seed in range(configs.num_rng_seeds):  # Reproducibility
        lr_finder = LearningRateFinder(
            min_lr=1e-6, max_lr=1e-2, early_stop_threshold=None
        )
...
def run_DMD():
    ...
    train_data, _, test_data = load_data()
    dmd_model = DMD(reduced_rank=configs.reduced_rank, rank=configs.classes).fit(
        train_data
    )
    oracle = load_oracle()
