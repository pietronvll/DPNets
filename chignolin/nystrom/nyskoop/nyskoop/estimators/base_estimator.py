...
class BaseNystromKoopmanEstimator(BaseEstimator, RegressorMixin):
    ...
    def eigenfunctions(self, what="eig"):
        check_is_fitted(
            self, ["ny_pts_x_", "ny_pts_y_", "U_", "V_", "fitted_components_"]
        )
        ...
        edec_left = Eigendecomposition(
            edec_left.eigenvalues, edec_left.eigenvectors.conj()
        )
    ...
    def modes(self):
        ...
        modes = (evec_left.conj() * left_right_norms.view(1, -1)).T @ to_complex(
            U.T @ ny_pts_y
        )
