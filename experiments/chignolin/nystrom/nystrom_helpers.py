...
def train_nystrom_est(X, Y, M, kernel, kind="krr", **kind_kwargs):
    ...
    if kind == "krr":
        ...
        est = ExactKoopmanNystromPcr(
            kernel=kernel, center_selection=cs_fixed, **kind_kwargs
        )
    elif kind == "fastpcr":
        est = RandomizedKoopmanNystromPcr(
            kernel=kernel, center_selection=cs_fixed, **kind_kwargs
        )
    elif kind == "rrr":
        est = ExactKoopmanNystromRrr(
            kernel=kernel, center_selection=cs_fixed, **kind_kwargs
        )
    elif kind == "fastrrr":
        est = RandomizedKoopmanNystromRrr(
            kernel=kernel, center_selection=cs_fixed, **kind_kwargs
        )
    else:
...
def gen_lagged(
    ...
    for i in range(num_test_sets):
        ...
        X_test = (
            full_data[test_set_start : test_set_start + n_test]
            .to(dtype=dtype)
            .contiguous()
        )
        Y_test = (
            full_data[test_set_start + q : test_set_start + n_test + q]
            .to(dtype=dtype)
            .contiguous()
        )
...
def nrmse(
    ...
    for i, (x_test, y_test) in enumerate(all_test_sets):
        ...
        nrmses.append(torch.sqrt(torch.mean(err, 0)) / torch.std(y_test, 0))
...
def rmse(
    ...
    for i, (x_test, y_test) in enumerate(all_test_sets):
        ...
        err = torch.square(y_pred - y_test)
        rmses.append(torch.sqrt(torch.mean(err, 0)))
